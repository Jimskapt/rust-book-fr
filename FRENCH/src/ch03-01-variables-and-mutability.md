...
par défaut, les variables sont _immuables_. C'est un des nombreux coups de pouce
...
générez un nouveau projet appelé _variables_ dans votre répertoire _projects_
en utilisant `cargo new variables`.
...
remplacez son code par le code suivant, qui ne se compile pas pour le moment :
...
avoir un message d'erreur à propos d'une erreur d'immutabilité, comme celui-ci :
...
signifient _pas_ que vous êtes un mauvais développeur ! Même les Rustacés
...
Vous avez reçu le message d'erreur `` impossible d'assigner à deux reprises la
variable immuable `x` `` (`` cannot assign twice to immutable variable `x` ``)
parce que vous avez tenté d'assigner une seconde valeur à la variable immuable 
`x`.
...
lorsque la seconde partie du code ne modifie que _parfois_ cette valeur. Le
...
code. Bien que les variables sont immuables par défaut, vous pouvez les rendre
mutables en ajoutant `mut` devant le nom de la variable , comme vous l'avez
fait au [chapitre 2][storing-values-with-variables]<!-- ignore -->.  L'ajout de
`mut` va aussi signaler l'intention aux futurs lecteurs de ce code que d'autres
parties du code vont modifier la valeur de cette variable.

<!--
For example, let’s change *src/main.rs* to the following:
-->

Par exemple, modifions _src/main.rs_ ainsi :
...
En utilisant `mut`, nous avons permis à la valeur liée à `x` de passer de `5`
à `6`.  En fin de compte, c'est à vous de décider si vous souhaitez utiliser la
mutabilité ou non, en fonction de ce qui vous semble le plus clair dans chaque
...
`let`, et le type de la valeur _doit_ être indiqué. Nous allons aborder les
...
disent que la première variable est _masquée_ par la seconde, ce qui signifie
que la seconde variable sera ce que le compilateur verra lorsque vous utilisez
le nom de cette variable.  En effet, la deuxième variable masque la première,
s'appropriant toutes les utilisations du nom de variable jusqu'à ce qu'elle
soit elle-même masquée ou que la portée prenne fin.  Nous pouvons masquer une
variable en utilisant le même nom de variable et en réutilisant le mot-clé
...
Au début, ce programme lie `x` à la valeur `5`. Puis il crée une nouvelle
variable `x` en répétant `let x =`, en récupérant la valeur d'origine et lui
ajoutant `1`, de sorte que la valeur de `x` est désormais `6`. Ensuite,
à l'intérieur de la portée interne créée par les parenthèses, la troisième
instruction `let` masque aussi `x` et crée une nouvelle variable, en
récupérant la précédente valeur et en la multipliant par `2` pour donner à `x`
la valeur finale de `12`. Dès que nous sortons de cette portée, le masque prend
fin, et `x` revient à la valeur `6`. Lorsque nous exécutons ce programme, nous
obtenons ceci :
...
La première variable `espaces` est du type chaîne de caractères _(string)_ et
...
<!--
GARBAGE BIN (au cas où): text erased from upstream; kept here just in case,
                         and eventually to be erased:
