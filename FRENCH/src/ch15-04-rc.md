...
Souvenez-vous que nous l'avons définie en utilisant `Box<T>`. Cette fois-ci, nous
...
données comme le font la plupart des implémentations de `clone`. L'appel à
...
augmenter le compteur : l'implémentation du trait `Drop` réduit le compteur de
