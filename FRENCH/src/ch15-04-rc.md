...
même nœud, et ce nœud est par conception possédé par toutes les extrémités qui
pointent vers lui. Un nœud ne devrait pas être nettoyé, à moins qu'il n'ait plus
...
déterminer si la valeur est toujours utilisée ou non. S'il y a zéro référence
...
À la place, nous allons changer la définition de `List` pour utiliser `Rc<T>` à
...
la liste qui stocke `5` et `10` et la stockons dans une nouvelle `Rc<List>` dans
...
### Cloner une `Rc<T>` augmente le compteur de références
...
À chaque étape du programme où le compteur de références change, nous affichons
